...
import Congratsmodal from '../../../components/assignment/congratsmodal.jsx';
...
if (isValid){
    ...
    setSubmission(true)
            ...
            let end = date.end    
            let assignment = {CourseId, startDate:start, endDate:end,title, ClassId, CompilerId, plagiarism, documentation, codingStandards, objectives, repository,readme, gitMode}
  ...
  return (
   ...
   <Congratsmodal modal={modal} toggleModal={toggleModal} assignmentId={assignmentID} compiler={comp}/>
                    ...
                    <form className="needs-validation" ref={formRef} onSubmit={handleSubmit}  noValidate>
                      ...
                      <div className="mb-3">
                        ...
                        setStartSpiner(false)
                          ...
                          <option disabled hidden  value='' selected className='text-muted'>program</option>
                          {programs.map(val => {
                        ...
                        disabled={loadingClass} onChange={async (val) => {
                            ...
                            <option hidden disabled selected value=''  className='text-muted'>{startSpiner? "loading...":"Select Class"}</option>
                            {Clases.map(val => {
                        ...
                        <select className="form-select" id="select-input"disabled={loadingCourses} onChange={async (val) => {
                            ...
                            <option hidden disabled value='' selected>Select Course</option>
                            {courses.map(val => {
                                return(<option key={val.id} value={val.id}>{val.courseName}({val.courseCode})</option>)
                            })}
                        </select>                      </div>
                        ...
                        <select className="form-select" id="select-input"  required onChange={(val) => {
                          ...
                          <option disabled hidden  value=''  selected className='text-muted'>programming language</option>
                          {compilers.map(val => {
